        ...
        <div className="flex flex-col gap-16">
          {resumeData.experience.map((exp, idx) => (
            ...
            >
              ...
              <div className={`${idx % 2 === 0 ? "md:text-right md:pr-12" : "md:order-2 md:pl-12"}`}>
                ...
                <h3 className="text-xl md:text-2xl font-display font-black text-white tracking-tight">{exp.role}</h3>
                  ...
                  exp.projects.map((proj) => (
                    ...
                    >
                      <div className="flex items-start justify-between gap-3 mb-3">
                        <h4 className="text-base font-display font-black text-white flex items-center gap-2 flex-wrap">
                          {proj.name}
                          {proj.role === "Team Lead" && (
                            <span className="px-2 py-0.5 rounded text-[9px] font-black bg-amber-500/10 text-amber-400 border border-amber-500/20 uppercase tracking-widest font-display">
                              {proj.role}
                            </span>
                          )}
                        </h4>
                      </div>
                      <p className="text-gray-400 text-xs leading-relaxed mb-3">{proj.description}</p>
                      <ul className="space-y-2">
                        {proj.details.map((d, i) => (
                          <li key={i} className="text-gray-300 text-xs flex items-start gap-2.5 leading-relaxed">
                      ...
                      <button
                        ...
                        className="mt-4 inline-flex items-center gap-1.5 text-[10px] font-bold text-violet-300 hover:text-violet-200 transition-colors cursor-pointer"
                      >
          ...
          >
            ...
            <div className="md:max-w-3xl md:mx-auto">
              ...
              <div className="glass rounded-2xl p-6">
                <h3 className="text-base font-display font-black text-white flex items-center gap-3 mb-5 justify-center">
